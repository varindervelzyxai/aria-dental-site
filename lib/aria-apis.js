...
Speak like a calm, sharp front-desk coordinator. Short paragraphs. No markdown. No bullet lists. 1–2 short sentences. Lead with the answer in the first sentence. No fake case studies or invented practice names.
...
WHAT ARIA DOES
- Answers the practice line 24/7, books appointments, verifies insurance live via eligibility networks, can collect payment, writes into Google Calendar or Open Dental (Dentrix and Eaglesoft on the roadmap). Calendar sync is not native PMS write-back.
...
- Conversational support across 100+ languages, with 17 production-tuned public voices across English, Spanish, Chinese and Hindi. Or clone the doctor's voice from a ~60-second sample.
- HIPAA: production Voice AI is deployed under a BAA. We are not SOC 2 certified. Never invent customer metrics or exclusive-competitor claims.
...
Return JSON only, keys in this exact order so speech can start before chips/go finish:
...
function buildFishTtsPayload(text, sampleRate) {
  return {
    text: text,
...
function fishAudioHeaders(sampleRate) {
  return {
...
function unescapeJsonTextPrefix(raw) {
  const s = String(raw || "");
  const key = s.match(/"text"\s*:\s*"/);
  if (!key) return { text: "", closed: false };
...
function extractClosedTextField(raw) {
  const got = unescapeJsonTextPrefix(raw);
  return got.closed ? got.text : null;
}

function extractSpeakableLead(raw) {
  const got = unescapeJsonTextPrefix(raw);
  const text = String(got.text || "").replace(/\s+/g, " ").trim();
  if (!text) return null;
  const sentence = text.match(/^(.{12,}?[.!?])(?:\s|$)/);
  if (sentence) return sentence[1].trim();
  return got.closed && text.length >= 8 ? text : null;
}
...
async function handleFishTts(payload) {
  ...
  const sampleRate = Math.min(Number(payload.sampleRate) || 24000, 44100);
  const key = fishKey();
  ...
  const res = await fetch(FISH_TTS, {
    ...
    body: JSON.stringify(buildFishTtsPayload(clipped, sampleRate)),
  });
  ...
  if (res.body) {
    return {
      status: 200,
      headers: fishAudioHeaders(sampleRate),
      stream: res.body,
    };
  }
...
function assembleGuideReply(raw, fallback, messages, model) {
...
async function streamGuideChat(openaiMessages, fallback, messages, model, key) {
  async function startStream(useModel, useJson) {
    var body = {
      model: useModel,
      temperature: 0.6,
      max_tokens: 240,
  ...
  var usedModel = model;
  var res = await startStream(usedModel, true);
  if (!res.ok) res = await startStream(usedModel, false);
  if (!res.ok && usedModel !== "gpt-4o-mini") {
    res = await startStream("gpt-4o-mini", true);
    if (res.ok) usedModel = "gpt-4o-mini";
  }
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      try {
        ...
        while (true) {
          ...
          for (var i = 0; i < parts.length; i++) {
            ...
            try {
              ...
              var piece = evt && evt.choices && evt.choices[0] && evt.choices[0].delta && evt.choices[0].delta.content;
              if (piece) {
                raw += piece;
                if (!emittedPartial) {
                  var early = extractSpeakableLead(raw);
                  if (early) {
                    emittedPartial = true;
                    controller.enqueue(encoder.encode(sseLine({ partial: early })));
                  }
                }
              }
            } catch (_) {}
        ...
        var reply = assembleGuideReply(raw, fallback, messages, usedModel);
        controller.enqueue(encoder.encode(sseLine(reply)));
        controller.close();
      } catch (e) {
        fallback.brain = "knowledge";
        fallback.reason = "openai_stream_error";
        fallback.detail = String((e && e.message) || e).slice(0, 180);
        controller.enqueue(encoder.encode(sseLine(fallback)));
        controller.close();
      }
    },
  });
...
async function handleGuideChat(payload) {
  ...
  if (payload && payload.stream) {
    try {
      var streamed = await streamGuideChat(openaiMessages, fallback, messages, openaiModel(), key);
  ...
  try {
    ...
    return json(200, assembleGuideReply(raw, fallback, messages, model));
  } catch (e) {
    fallback.brain = "knowledge";
...
function pumpWebStreamToNode(webStream, res) {
...
function applyVercel(res, result) {
  ...
  if (result.stream) {
    if (typeof result.stream.getReader === "function") {
      pumpWebStreamToNode(result.stream, res);
      return;
    }
...
module.exports = {
  ...
  handleGuideChat,
  buildFishTtsPayload,
  extractClosedTextField,
  extractSpeakableLead,
  assembleGuideReply,
  readJsonBody,
  applyVercel,
  toNetlify,
