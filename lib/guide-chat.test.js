...
async function main() {
  ...
  const streamed = await handleGuideChat({
    messages: [{ role: "user", content: "show me the voices" }],
    path: "/",
    stream: true,
  });
  assert(streamed.status === 200, "stream flag without key");
  assert(!streamed.stream, "no-key stream stays JSON");
  const voicesStream = JSON.parse(streamed.body);
  assert(voicesStream.go && voicesStream.go.path === "/voices/", "stream fallback still walks voices");
